...
const requiredPrivateRoutes=['/auth','/auth/confirm','/auth/error','/account','/studio','/studio/signals','/newsletter/confirm','/newsletter/unsubscribe']
...
function routeExists(route){
  ...
  for(const part of parts){
    ...
    for(const base of candidates){
      ...
      for(const entry of fs.readdirSync(base,{withFileTypes:true}))if(entry.isDirectory()&&/^\[.*\]$/.test(entry.name))next.push(path.join(base,entry.name))
    }
    candidates=next
...
for(const file of sourceFiles){
  ...
  for(const match of text.matchAll(re)){
    ...
    if(!routeExists(route))errors.push(`Broken static internal href ${href} in ${path.relative(root,file)}`)
  }
}
...
console.log(`KAPORAL launch QA passed: ${requiredPublicRoutes.length} public routes, ${requiredPrivateRoutes.length} protected/transactional routes, ${hrefCount} static internal links checked.`)
