...
import { IntelligenceProductEditor } from '../../../../components/IntelligenceProductEditor'
...
export default async function ArticleWorkspacePage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="studio-shell researchWorkspace"><header className="studio-head"><div><p className="eyebrow">KAPORAL RESEARCH OS · EDITORIAL REVIEW</p><h1>{data.article.headline}</h1><p>{data.article.status} · {data.article.reader_level} · {data.article.confidence??'confidence pending'}</p></div><div className="workspaceActions"><Link href={data.article.story_candidate_id?`/studio/story/${data.article.story_candidate_id}`:'/studio'}>← Research pack</Link><Link href="/track-record">Track Record</Link><Link href="/studio">Command center</Link></div></header>
  ...
  <IntelligenceProductEditor articleId={data.article.id} role={data.profile.role} predictions={data.predictions as any[]} impactMap={data.impactMap as any} impactNodes={data.impactNodes as any[]} impactEdges={data.impactEdges as any[]}/>
 </main>
}
