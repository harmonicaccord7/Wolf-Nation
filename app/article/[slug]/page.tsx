...
export default async function ArticlePage({params}:{params:Promise<{slug:string}>}){
 ...
 const {data:article}=await supabase.from('articles').select('id,headline,dek,body,confidence,reader_level,published_at,updated_at,disclaimer_variant').eq('slug',slug).maybeSingle()
 if(!article) notFound()
 const [{data:claims},{data:links},{data:predictions},{data:corrections},{data:map}]=await Promise.all([
   supabase.from('claims').select('id,claim_text,claim_type,confidence,verification_status').eq('article_id',article.id),
   supabase.from('article_sources').select('claim,note,source:sources(title,publisher,url,source_type,published_at,reliability_score)').eq('article_id',article.id),
 ...
 return <main className="investigation"><header className="investigationNav"><Link href="/">← KAPORAL INTELLIGENCE</Link><span>Independent research & education · Not financial advice</span></header><article>
   ...
   </div><aside className="investigationAside"><div className="asideCard"><small>OUR RULE</small><h3>Explain. Don’t command.</h3><p>This publication does not provide personalized financial advice. Verify sources, assumptions and suitability before making any financial decision.</p></div><div className="asideCard"><small>CLAIM CHECK</small><h3>{claims?.length??0} structured claims</h3>{claims?.slice(0,6).map((c:any)=><div className="claimRow" key={c.id}><span>{c.verification_status}</span><p>{c.claim_text}</p></div>)}</div><div className="asideCard"><small>SOURCES</small><h3>{links?.length??0} linked sources</h3>{links?.map((l:any,i)=><a className="sourceRow" key={i} href={l.source?.url} target="_blank" rel="noreferrer"><b>{l.source?.publisher??'Source'}</b><span>{l.source?.title??l.claim??'Evidence'}</span></a>)}</div></aside></section>
