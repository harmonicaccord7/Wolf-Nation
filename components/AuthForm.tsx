  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      if(mode==='signup'){
        ...
        if(data.user && data.session){ await supabase.from('profiles').insert({id:data.user.id,display_name:email.split('@')[0],role:'reader'}) }
        ...
        if(data.user){ await supabase.from('profiles').insert({id:data.user.id,display_name:email.split('@')[0],role:'reader'}).then(()=>{}) }
