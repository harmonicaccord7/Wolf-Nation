...
export async function DeskPage({slug}:{slug:DeskSlug}){
  ...
  return <main className="intelligencePage">
    ...
    {slug==='africa'?<section className="shell liveSection"><div className="liveSectionHead"><div><span className="eyebrow">AFRICA MACRO MATRIX</span><h2>Country-by-country evidence</h2></div><p>Stored annual indicators, not live quotes. Compare the reporting year before comparing countries. Missing data is not zero.</p></div><div className="africaCountryGrid">{[...africaGroups.entries()].map(([code,group])=><article className="africaCountryCard" key={code}><div className="countryCardHead"><strong>{group[0]?.label.split(' — ')[0]??code}</strong><span>{code}</span></div>{group.map(m=><div className="countryMetric" key={m.code}><span>{m.label.split(' — ')[1]??m.label}</span><b>{formatIntelligenceValue(m.latest?.value,m.unit)}</b><small>{m.latest?.provider??'Unavailable'} · {formatObservationDate(m.latest?.observedAt,m.frequency)}</small>{m.unit==='LCU per USD'&&<small>Annual average · local currency units per US$1; not a live FX quote.</small>}<Link href={`/data/${m.code.toLowerCase()}`}>History →</Link>{m.sourceUrl&&<a href={m.sourceUrl} target="_blank" rel="noreferrer" aria-label={`${m.label} source`}>Source ↗</a>}</div>)}</article>)}</div></section>:metrics.length>0?<section className="shell liveSection"><div className="liveSectionHead"><div><span className="eyebrow">MARKET DATA & PUBLISHED INDICATORS</span><h2>Verified data, not decorative numbers.</h2></div><p>{config.sourceNote}</p></div><div className="liveMetricGrid">{metrics.map(m=><MetricCard key={m.code} metric={m}/>)}</div></section>:null}

    {slug==='options'&&<OptionsLab metrics={metrics}/>}    
    {slug==='learn'&&<LearnPath/>}
    {slug==='business'&&<BusinessFramework/>}
    {slug==='technology'&&<TechnologyFramework/>}
