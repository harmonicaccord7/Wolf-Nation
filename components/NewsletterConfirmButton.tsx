...
export function NewsletterConfirmButton({token}:{token:string}){
  ...
  return <div className="authConfirmAction"><button className="goldButton big" type="button" onClick={confirm} disabled={busy} aria-busy={busy}>{busy?'Confirming…':'Confirm Market Letter'}</button>{status==='error'&&<div className="authStatus" role="status"><p>This confirmation token is invalid or expired. Submit your address again to request a fresh email.</p><Link href="/#newsletter">Request a new confirmation →</Link></div>}</div>
}
