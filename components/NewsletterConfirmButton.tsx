...
export function NewsletterConfirmButton({token}:{token:string}){
  ...
  return <div className="authConfirmAction"><button className="goldButton big" type="button" onClick={confirm} disabled={busy} aria-busy={busy}>{busy?'Confirming…':'Confirm subscription'}</button>{status==='error'&&<div className="authStatus" role="status"><p>{invalidLink?'This confirmation token is invalid or expired. Submit your address again to request a fresh email.':'We could not confirm your subscription right now. Please try this button again in a moment.'}</p>{invalidLink&&<Link href="/newsletter#newsletter-signup">Request a new confirmation →</Link>}</div>}</div>
}
