...
export async function getSignalLab(){
  ...
  const [{data:models},{data:predictions},{data:outcomes},{data:backtests}]=await Promise.all([
    ...
    supabase.from('backtest_runs').select('id,model_version_id,universe,period_start,period_end,train_end,observations,hit_rate,precision_score,brier_score,avg_return_pct,max_drawdown_pct,assumptions,evaluation:results->test->model,benchmark:results->test->baseline,uncertainty:results->test->brierDifference,active_days:results->test->strategies->0->model->activeDays,created_at').order('created_at',{ascending:false}).limit(30)
  ])
