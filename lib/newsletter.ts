...
export async function newsletterAction(action:'subscribe'|'confirm'|'unsubscribe',payload:Record<string,unknown>):Promise<NewsletterResult>{
 ...
 try{
  const service=action==='unsubscribe'&&typeof payload.token==='string'&&payload.token.startsWith('d.')?'newsletter-delivery':'newsletter'
  const response=await fetch(`${base}/functions/v1/${service}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,...payload}),cache:'no-store'})
